...
const BRAND = 0x00d4ff;  // electric cyan — matches the dashboard theme
...
export function buildStatusEmbed(rooms: RoomSummary[]): EmbedBuilder {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(`**${totalOn}** of **${totalAll}** devices active · **${formatWatts(totalWatts)}** live draw`)
    .setTimestamp();
...
export function buildRoomEmbed(room: RoomSummary): EmbedBuilder {
  ...
  const pct     = maxLoad > 0 ? Math.round((room.totalPowerDraw / maxLoad) * 100) : 0;
  const bar     = '█'.repeat(Math.round(pct / 10)) + '░'.repeat(10 - Math.round(pct / 10));
  const color   = pct === 100 ? Colors.Red : pct > 50 ? Colors.Yellow : BRAND;

  return new EmbedBuilder()
    .setColor(color)
    .setTitle(`📍  ${room.room}`)
    .setDescription(
      `**${room.onCount}** of **${room.deviceCount}** devices ON  ·  **${formatWatts(room.totalPowerDraw)}** live\n` +
      `\`${bar}\` ${pct}% load`
    )
    .addFields(
...
export function buildUsageEmbed(usage: UsageSummary): EmbedBuilder {
  const roomFields = usage.rooms.map((r) => {
    const share = usage.totalPowerDraw > 0 ? Math.round((r.powerDraw / usage.totalPowerDraw) * 100) : 0;
...
export function buildHelpEmbed(prefix: string): EmbedBuilder {
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      { name: cmd('status', '',            'All 15 devices grouped by room with live wattage'),               value: '​', inline: false },
      { name: cmd('room',   '<name>',      'Detailed view of one room. Name can be partial (e.g. `work 1`)'), value: '​', inline: false },
      { name: cmd('usage',  '',            'Live power consumption totals and per-room breakdown'),            value: '​', inline: false },
      { name: cmd('alerts', '',            'All active alerts — after-hours devices and sustained load'),      value: '​', inline: false },
      { name: cmd('ask',    '<question>',  'Ask the AI assistant anything about the office'),                 value: '​', inline: false },
      { name: cmd('help',   '',            'Show this message'),                                               value: '​', inline: false },
    )
